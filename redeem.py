...
redeem = bot.create_group("redeem", "Redeem gift codes", guild_ids=[742405919249268767, 742269996071387238])

@redeem.command(name="genshin", description="Redeem for Genshin Impact", guild_ids=[742405919249268767, 742269996071387238])
async def genshin(ctx: discord.ApplicationContext, code: str):
    ...
    if not user:
        embed.add_field(name=f"Error", value=f"I don't have your credit card details", inline=False)
    else:
        user = user[0]
